...
cout<<"Lambda_4: "<<lambda4<<endl;	*/
        ...
        for (e=0;e<m;e++){
			...
			IloInt u = min(a,b);
			IloInt v = max(a,b);
			mauxOpt.add(R[e] >= AA[u] - AA[v]);
			mauxOpt.add(R[e] >= AA[v] - AA[u]);
			mauxOpt.add(R[e] <= AA[v] + AA[u]);
			mauxOpt.add(R[e] <= 2 -AA[v] - AA[u]);
		}*/
...
cout<<"****************************"<<endl;        */
		...
		//l(4)
		/*bool existe = false;
		for(int tt=0; tt< A.getSize()-1; tt++){
			bool tudoIgual = true;
			for(int v=0; v<n; v++){
				if (A[tt][v] != A[t][v]){
					tudoIgual = false;
		...
		if (existe == false)*/ consLambda4[0].setExpr(consLambda4[0].getExpr()+ Z[t]);
